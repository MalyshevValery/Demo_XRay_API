...
def normalize_by_lung_quantiles(img, mask, q=5):
    lung_intensities = img.flatten()
    lung_intensities = lung_intensities[mask.flatten() > 0]
    q1 = np.percentile(lung_intensities, q)
    q2 = np.percentile(lung_intensities, 100 - q)
...
def normalize_by_lung_mean_std(img, mask):
    lung_intensities = img.flatten()
    lung_intensities = lung_intensities[mask.flatten() > 0]
    mn = lung_intensities[::4].mean()
    std = lung_intensities[::4].std()
    img -= mn
    img /= std * 7.
    img += 0.5
    return img
...
def normalize_by_lung_convex_hull(img, mask, q=5):
